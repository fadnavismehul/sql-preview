...
import BigQueryConnector from './index';

async function runMcpServer() {
    const server = new Server(
        { name: 'sql-preview-bigquery', version: '1.0.0' },
        { capabilities: { tools: {} } },
    );
        ...
        tools: [
            {
                name: 'execute_query',
                description:
                    'Execute a SQL query against Google BigQuery and return results as JSON.',
                inputSchema: {
                    type: 'object',
                    properties: {
                        query: { type: 'string', description: 'Standard SQL query to execute.' },
...
async function runCli() {
    ...
    if (!query) {
        console.error(
            'Usage: sql-preview-bigquery [--mcp] | [--query <sql> --config <base64_json>]',
        );
        process.exit(1);
    }
