...
async function runMcpServer() {
  ...
  const connector = new PostgreSQLConnector(null);
      ...
      tools: [
        {
          name: 'execute_query',
          description:
            'Executes a SQL query against PostgreSQL and returns the results as JSON chunks over MCP.',
          inputSchema: {
            type: 'object',
            properties: {
...
async function runCli() {
  ...
  if (!query) {
    console.error('Usage: sql-preview-postgres [--mcp] | [--query <sql> --config <base64_json>]');
    process.exit(1);
  }
