...
import SnowflakeConnector from './index';

async function runMcpServer() {
    const server = new Server(
        { name: 'sql-preview-snowflake', version: '1.0.0' },
        { capabilities: { tools: {} } },
    );
        ...
        tools: [
            {
                name: 'execute_query',
                description:
                    'Execute a SQL query against a Snowflake data warehouse and return results as JSON.',
                inputSchema: {
                    type: 'object',
                    properties: {
...
async function runCli() {
    ...
    if (!query) {
        console.error(
            'Usage: sql-preview-snowflake [--mcp] | [--query <sql> --config <base64_json>]',
        );
        process.exit(1);
    }
