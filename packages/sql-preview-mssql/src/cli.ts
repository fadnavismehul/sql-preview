...
import MSSQLConnector from './index';

async function runMcpServer() {
    const server = new Server(
        { name: 'sql-preview-mssql', version: '1.0.0' },
        { capabilities: { tools: {} } },
    );
        ...
        tools: [
            {
                name: 'execute_query',
                description:
                    'Execute a SQL query against Microsoft SQL Server or Azure SQL Database and return results as JSON.',
                inputSchema: {
                    type: 'object',
                    properties: {
...
async function runCli() {
    ...
    if (!query) {
        console.error(
            'Usage: sql-preview-mssql [--mcp] | [--query <sql> --config <base64_json>]',
        );
        process.exit(1);
    }
