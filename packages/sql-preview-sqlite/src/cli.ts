...
async function runMcpServer() {
  ...
  const connector = new SQLiteConnector(null);
      ...
      tools: [
        {
          name: 'execute_query',
          description:
            'Executes a SQL query against SQLite and returns the results as JSON chunks over MCP.',
          inputSchema: {
            type: 'object',
            properties: {
...
async function runCli() {
  ...
  if (!query) {
    console.error('Usage: sql-preview-sqlite [--mcp] | [--query <sql> --config <base64_json>]');
    process.exit(1);
  }
