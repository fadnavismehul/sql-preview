...
5.  **Agent Decision**:
    *   *Path A (Analysis)*: "Oh, it's huge. I'll run `SELECT count(*) ...` instead." (Success: Compute pushed to DB)
    *   *Path B (Inspection)*: "I need to see the next 50 rows." calls `get_tab_info(mode='page', limit=50, offset=5)`
    *   *Path C (Export)*: "I need full access." calls `read_resource(resourceUri)`
