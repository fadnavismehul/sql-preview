...
Instead of inventing a custom JSON protocol, we utilized the official **Model Context Protocol (MCP)** and run it over a WebSocket connection. The Daemon implements a robust `DaemonMcpServer` and `DaemonMcpToolManager` containing tools like `run_query` and `get_tab_info` exposed simultaneously via SSE, Stdio, and now WebSockets.

### 1. WebSocket Transport for the Existing MCP Server (The Interface)

- **Handshake:** The agent connects strictly to `ws://localhost:XXXX/mcp/ws`.
- **Authentication:** Standard authentication tokens can be passed in initial headers or as a first initialization message.
- **Bi-directional JSON-RPC:** The connection transports standard MCP JSON-RPC 2.0 messages. Agents can instantly use existing tools (`run_query`, `list_sessions`) and subscribe to resource changes without any additional custom API client logic.

### 2. Dual-Track Data Plane (The Engine)

For heavy analytical workloads, JSON-RPC (even over WebSockets) will choke on millions of rows. We established a dual-track system working in tandem with RFC-013:
...
The WebSocket transport was successfully built directly into the Native Express lifecycle using the `@modelcontextprotocol/sdk`. Benchmarks proved heavily effective:

- **Payload:** 1KB JSON payload mapping (tools enumeration)
- **Throughput:** ~19.8 MB/s sustained high-load throughput
- **Latency (RTT):** ~0.20ms average RTT / 0.96ms p99 latency
